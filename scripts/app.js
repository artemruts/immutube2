...
, 'http'
  ...
  var compose = P.compose,
      map = P.map,
      log = function(x) { console.log('LOG => ', x); return x; },
      fork = _.curry(function(f, future) { return future.fork(log, f); }),
      setHtml = _.curry(function(sel, x) { return $(sel).html(x); });
  ...
  // PURE //////////////////////////////////////////////////////////////////////////////
   ...
   var targetValue = (compose(_.get('value'), _.get('target')));
   ...
   var createLiFromItems = compose(map(createLi), _.get('items'));
   ...
   var getVideoId = compose(Maybe, _.get('youtubeid'), _.get('dataset'));
